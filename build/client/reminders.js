...
const makeListElement = n => {
  ...
  const li = $(`<li class="plugin-calendar-event-editor-reminder" data-value="${n}">
      ${zero.to(n, true)}
      <a class="remove" href="#">
...
const factory = $ul => {
  const addButtons = $ul.find('#plugin-calendar-event-editor-reminders-add li > a');

  const methods = {
    setReminders: reminders => {
      $ul.find('li.plugin-calendar-event-editor-reminder').remove();
      $ul.data('value', reminders);
      reminders.reverse().map(makeListElement).forEach(el => $ul.prepend(el));
    },
    ...
    addReminder: n => {
      ...
      const reminders = [].concat((0, _toConsumableArray3.default)(nots), [n]).sort((a, b) => b - a);
